...
game.catchPokemon = function (pokemonName) {
  ...
  for (let i=0; i<pokemon.length; i++){
    let pokemonNameArray = pokemon[i].name.toLowerCase()

    if (pokemonNameArray === pokemonName){
...
for(let i=0; i<pokemon.length; i++){
    if(!pokemonTypeObject[pokemon[i].type]){
      pokemonTypeObject[pokemon[i].type] = []
    }
    pokemonTypeObject[pokemon[i].type].push(pokemon[i])

}
console.log("Exercise 21: ")
console.dir(pokemonTypeObject, {'maxArrayLength': null})
